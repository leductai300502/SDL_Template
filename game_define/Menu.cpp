...
bool Menu::LoadMenu(){
    bool isGameTitleSuccess = GameTitle.LoadImage(MENU_GAME_NAME,g_Screen,0,0);
    bool isSinglePlayerModeSuccess = SinglePlayerMode.LoadImage(MENU_SINGLEPLAYER_BTN,g_Screen,0,0);
    bool isMultiPlayerModeSuccess = MultiPlayerMode.LoadImage(MENU_MULTIPLAYER_BTN,g_Screen,0,0);
    bool isOptionsModeSuccess = OptionsMode.LoadImage(MENU_OPTIONS_BTN,g_Screen,0,0);
    bool isCreditsModeSuccess = CreditsMode.LoadImage(MENU_CREDITS_BTN,g_Screen,0,0);
    bool isQuitModeSuccess = QuitMode.LoadImage(MENU_QUIT_BTN,g_Screen,0,0);
...
void Menu::RunMenu(){
    ...
    if(g_Event.button.button==SDL_BUTTON_LEFT){
        if(g_Event.button.type == SDL_MOUSEBUTTONDOWN){
